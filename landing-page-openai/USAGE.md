...
이 문서는 Landing Page Agent (OpenAI)의 상세한 사용 방법을 설명한다.
...
**해결 방법**:
1. OpenAI 대시보드에서 사용량을 확인한다
2. 잠시 대기 후 재시도한다
3. `--max-retries` 옵션을 늘린다
4. API 사용 한도를 늘린다
...
**해결 방법**:
1. `.env` 파일에서 `MAX_TOKENS`를 줄인다
2. Skill 문서의 크기를 줄인다
3. 더 긴 컨텍스트를 지원하는 모델을 사용한다 (gpt-4-turbo-preview)
...
`.env` 파일에서 다음 값들을 조정하여 최적화한다:

```bash
MODEL_NAME=gpt-4-turbo        # 모델 선택
...
- `gpt-4-turbo`를 사용한다 (gpt-4보다 저렴)
- `MAX_TOKENS`를 적절히 조정한다
- 불필요한 재생성을 피한다
...
```bash
# .env 파일
MAX_MEMORY_SIZE=50  # 기본값: 50개 메시지
```
...
내부적으로 asyncio를 사용하여 효율적으로 처리한다.
